...
async function main(argv: string[]): Promise<number> {
  ...
  if (cmd === "run") {
    ...
    if (!commandArgs.length) {
      ...
      for (let i = 0; i < flags.length; i++) {
        const a = flags[i];
        if (a === "--label" || a === "--adapter" || a === "--agent-output" || a === "--pty" || a === "--project") {
          f.push(a, flags[i + 1] || "");
          i++;
  ...
  if (cmd === "adapter") {
    ...
    if (sub === "test") {
      ...
      if (!target) {
        process.stderr.write("usage: flowpeek adapter test <id|path> [--fixture file] [--json]\n");
        return 2;
      }
      return ad.cmdAdapterTest({ target, fixture, json, project });
    }
    if (sub === "scaffold") {
      const id = restA[0];
